...
export default CustomizationMex;
const EditCondimentosAdicionales = (prop) =>{
    const [bebSelected2,setBeb2]=useState(null)
    const [isSelected, setIsSelected]=useState(false)
    const bebidas2=([
        {title:"Tocino", price:"25.00"},
        {title:"Queso Americano", price:"20.00"},
        {title:"Salsa fasfú (tiene pepinillos)", price:"20.00"},
        {title:"Mermelada cebolla y chile", price:"15.00"},
        {title:"Mermelada tocino whisky", price:"22.50"},
    ])
...
const EditPSAdicionales = (prop) =>{
    ...
    const [isSelected, setIsSelected]=useState(false)
    const bebidas2=([
        {title:"Lemon paradise", price:"60.00"},
        {title:"Galleta tahine", price:"25.00"},
        {title:"Cremoso chocolate", price:"60.00"},
    ])
    const [selectedAdditionalBeb,setAdditionalBeb]=useState("")
...
const EditPapasAdicionales = (prop) =>{
    ...
    const [isSelected, setIsSelected]=useState(false)
    const bebidas2=([
        {title:"Big fries", price:"75.00"},
        {title:"Fries", price:"50.00"},
        {title:"Big fries queso y tocineta", price:"95.00"},
    ])
    const [selectedAdditionalBeb,setAdditionalBeb]=useState("")
    ...
    return(
        <c.SectionView>
            <c.SectionTopTitleN>
                <c.SectionViewTitle>Añade acompañamientos</c.SectionViewTitle>
                <c.SectionViewDesc>Selecciona máximo 10 unidades</c.SectionViewDesc>
            </c.SectionTopTitleN>
            <c.SectionBody>
                {
...
const EditBebida = (prop) =>{
    ...
    const bebidas=([
        {title:"Coca-cola sabor original 400ml", price:"5.900"},
        {title:"Coca-cola sin azúcar 400ml", price:"5.900"},
        {title:"Agua natural ciel embotellada", price:"5.900"},
    ])
    useEffect(()=>{
        setBeb(0)
    }, [prop.isDefaultValues])
            ...
            <c.SectionBody>
                {
                    bebidas.map((beb,i)=>{
                        return(
                            <c.Section onClick={()=>{
                                prop.setAdditionalProducts(old=>[...old.filter((data)=>data.additionName != "Coca-cola sabor original 400ml" && data.additionName != "Coca-cola sin azúcar 400ml" && data.additionName != "Agua natural ciel embotellada"), {
                                    additionName:beb.title,
                                    id:prop.productId
                                }])
